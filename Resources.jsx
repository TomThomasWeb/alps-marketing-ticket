      ...
      <div key={tp.id} style={{ background: "var(--bg-card)", borderLeft: "4px solid " + st.color, borderRadius: 14, padding: "14px 18px", boxShadow: "0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 160 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: 6 }}>{tp.name}{stale && <span className="hub-breathe" style={{ fontSize: 9, fontWeight: 600, padding: "1px 6px", borderRadius: 20, background: "rgba(202,138,4,0.08)", color: "#ca8a04" }}>6+ months</span>}</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {updateCampaigns.map((camp) => { const done = camp.items.filter((i) => i.done).length; const total = camp.items.length; const pct = Math.round(done / total * 100); return (
              <div key={camp.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, padding: "18px 20px", opacity: camp.completed ? 0.5 : 1 }}>
                ...
                <div style={{ background: "var(--bg-input)", borderRadius: 6, height: 6, overflow: "hidden", marginBottom: 14 }}><div className="hub-bar-fill" style={{ height: "100%", background: pct === 100 ? "#16a34a" : pct > 50 ? "#ca8a04" : "#dc2626", borderRadius: 6, width: pct + "%" }}></div></div>
